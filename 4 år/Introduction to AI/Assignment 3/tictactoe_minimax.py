...
def minimax_search(game: Game, state: State) -> Action:
    ...
    # Call max_value for the current player to get the best action
    _, action = max_value(state)
    return action

# Self-play loop with total time measurement for Minimax
if __name__ == "__main__":
    ...
    while not game.is_terminal(state):
        ...
        # Start timer for first choice
        start_time = time.time()
        # Find the best action using Minimax (without pruning)
        action = minimax_search(game, state)
       
        end_time = time.time()
        total_time = end_time - start_time
        print(f"Total time taken for Minimax (without pruning): {total_time:.6f} seconds")
