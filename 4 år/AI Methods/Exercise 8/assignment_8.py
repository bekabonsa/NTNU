...
def feed_forward(x, hidden_W, hidden_b, out_W, out_b):
    ...
    Weighted_sum = x @ hidden_W + hidden_b
    hidden_activations = sigmoid(Weighted_sum)
...
def backward(y_hat, y, hidden_activations, x, out_W):
    ...
    # Step nr 1: Calculate loss
    # Loss = (y_hat-y)^2 
    #Loss derived with respect to y_hat, here 2 can be omitted because it doesnt change the direction of the gradient, only scales it
    d_L_d_yhat = y_hat-y 


    # gradients wrt output weights
    # A * dL/d_yhat (chain rule), last layer has linear activation f(x) = x
   
    d_L_d_ow = hidden_activations.T @ d_L_d_yhat  # Size (2,1)

    # gradients wrt output bias
    # 1 * d_L_d_yhat (chain rule)
    d_L_d_ob = np.sum(d_L_d_yhat, axis=0)  # Size (1,)
    
    # gradients wrt hidden weights
    d_L_d_hw = x.T @ ((d_L_d_yhat @ out_W.T)*(hidden_activations * (1-hidden_activations)))   # Size (2,2)
    # gradients wrt hidden bias
    d_L_d_hb = np.sum((d_L_d_yhat @ out_W.T) * (hidden_activations * (1 - hidden_activations)), axis=0)     # Size (2,)
...
def train(x_train, y_train, neural_net, learning_rate=0.01, epochs=10, batch_size=1):
    ...
    for e in range(epochs):
        ...
        for i in range(n_batches):
            ...
            print(f"Shape of hidden_b: {hidden_b.shape}")  # Expected (2,)
            print(f"Shape of d_L_d_hb: {d_L_d_hb.shape}")  # Should be (2,), but might be (1,2)
